...
def main():
    ...
    if FLAGS.test_mode=='2':
        cap=cv2.VideoCapture('/content/SemanticSegmentaition-DeepLabv3/street.mp4')
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(FLAGS.output+'/content/SemanticSegmentaition-DeepLabv3/out.mp4' ,fourcc,10,(1280,480))
        while True:
            ret,frame = cap.read()
            if ret == True:
